"""
aihydro-core jobs block — async execution substrate.

Relocated from ai_hydro/mcp/jobs.py (aihydro-tools). The original module
becomes a thin re-export shim for one release cycle to avoid breaking imports.
...
Contract:
    ...
    get_job_result(job_id)   -> final artifact or in-progress/error dict
    cancel_job(job_id)       -> kills PID's process group, marks cancelled
    list_jobs(kind=None)     -> registry view reconciled with status.json files

See AIHYDRO_CORE_DESIGN.md (local-docs/) §5 for the full contract spec.
"""
...
# --------------------------------------------------------------------------- #
# PID registry (single source of truth for PID + status)
# --------------------------------------------------------------------------- #
...
def _write_registry(reg: dict) -> None:
    ...
    tmp.replace(_REGISTRY)   # atomic on POSIX
...
def start_job(
    ...
    *,
    ...
    The feature-compute block uses this for batch fan-out (N features → N jobs).
    """
...
def get_job_status(job_id: str) -> dict:
    """Return the job's status.json, annotated with liveness if stale."""
    ...
    if status.get("status") not in _TERMINAL and entry and not _pid_alive(entry.get("pid")):
        ...
        status["_note"] = (
            "Process is no longer running but status is non-terminal — it may have crashed."
        )
    return status
...
def list_jobs(kind: str | None = None) -> dict:
    ...
    Includes Python-registry jobs AND TypeScript-spawned CLI subagents
